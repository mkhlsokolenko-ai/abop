...
async def run_live(agent: dict, contract: dict, safety_of, *, data_query, skill_sources,
    ...
    async def _analyze(sid):
        ...
        _task_verb = ("объясни ВЫЯВЛЕННЫЕ НАХОДКИ по методике навыка (суть/чем грозит/что проверить), опираясь "
                      "на данные; НЕ ищи новых и НЕ пиши «расхождений нет»" if explain
                      else "примени методику к данным и найди конкретные расхождения")
