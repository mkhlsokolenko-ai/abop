...
# Параллелизм LLM-вызовов навыков = 3 (компромисс). ЗАМЕР 2026-09-18: единая карта RouteAI троттлит
# при 5 → прогон в 2.7× МЕДЛЕННЕЕ (110с→299с); 2 — проверенный оптимум (110с). 3 — середина.
# Гипотеза «больше параллелизма быстрее» на одной карте опровергнута; реальный выигрыш дал дайджест.
# Backoff-ретрай оставлен для устойчивости. Поднимать выше имеет смысл только с несколькими картами/vLLM.
