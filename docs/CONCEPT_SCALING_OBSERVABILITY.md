...
Не сделано (следующие шаги): HITL-resume через очередь (§3.5), цепочки агентов (pipelines) по-прежнему синхронны в запросе, `abop_run_queue_depth` как gauge в Prometheus (сейчас только `/api/runs/queue`), Kafka-брокер и мультиноды (§11), per-tenant приоритеты (§11.2).
