...
**Фаза 0 — перенос + auth-фундамент:**
- Скопировать `desktop/` в репо ABOP (Electron/сайдкар/ui) без изменения ABOP-ядра.
- Выровнять realm; на проде ABOP включить строгую верификацию JWT (JWKS). Проверить: JWT desktop
  принимается и **верифицируется** ABOP; поддельный → 401.
- CORS/локальный токен сайдкара.

**Фаза 1 — тонкий прокси (минимальный вертикальный срез):**
- Добавить в сайдкар `abop_client.py` (base URL ABOP + проброс JWT).
- Модуль `agents`: заменить SQLite-каталог на прокси `/api/agents`; запуск → `demo/prepare`+`runs`;
  навыки → `/api/skills`. Результат прогона + HITL-очередь в UI.
- Демо-путь: в окне desktop выбрал агента → запустил → увидел находки → подтвердил HITL.
