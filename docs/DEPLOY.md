...
**Выделенный инстанс LLM (план):** подключить Qwen 30B из соседнего обучающего проекта — через `LOCAL_LLM_BASE_URL`/`LOCAL_LLM_MODEL` (vLLM). На выделенной карте параллелизм `ABOP_RUN_LLM_CONCURRENCY` можно поднимать (5-8+) без троттлинга RouteAI → перепроверить замер. Текущее значение на проде — `3` (env).
