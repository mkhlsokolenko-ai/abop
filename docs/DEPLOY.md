...
**Нагрузочное тестирование (2026-09-20, 1 uvicorn-воркер, 4 CPU, RAM в обрез):** «RPS» ≠ одно число — три класса. `/api/health` ~450 RPS. Auth+чтение: `/api/skills` **33→100-127 RPS** после кэша парсинга .md (lru_cache на parse_skill_md/load_skill_body); `/api/runs` список ~40-50 RPS (узкое место — N+1 обогащение по агентам + JWT-verify, оставлено на «с нодами»). Горячий multi-user (cached-прогоны) ~15/с на воркер, под admission=12 + rate-limit 30/300с/юзер (проверено: 429 после лимита). Cold-прогоны LLM-bound (~40с, кэш — множитель). Дешёвые app-выигрыши сделаны; тяжёлое (uvicorn --workers, шардинг, вынос rate-limit/admission в Redis) — при подъёме 2-3 доп. нод (per-worker in-memory state иначе размножит семафоры). Хост memory-tight (~426Mi доступно) — для нагрузочного прода нужна отдельная машина.

**Что помогло, а что нет (честно, замеры 2026-09-20):** реальный app-level выигрыш дал только кэш парсинга .md (`/api/skills` 33→100-127 RPS). JWT-verify-кэш (safe: хэш токена, TTL≤exp, поддельный→401) и фикс N+1 в `/api/runs` (лёгкие сводки вместо full-get с graph) — корректные улучшения, но RPS `/api/runs` не сдвинули (~51): его потолок при конкуренции — сам ОДИН uvicorn-воркер (GIL: сериализация ~81 прогона; PG-пул max_size=8 → ждут соединение), а не per-request работа. Эти два фикса дадут эффект при многоворкерности/нодах. Дальнейший рост `/api/runs` = `--workers` + больше PG-пул/CPU (уровень нод).
