...
AGENT_FAMILIES = {
    ...
    "audit": {
        "title": "Аудит", "profile": "research",
        "mission": "Проверяй учётные данные детерминированно: строй граф связей по реальным ссылкам, прогоняй проверки инвариантов/цепочек/НСИ/норм — находки считает код с доказательством, объяснение опирается на НК/ФСБУ, наружу ничего без HITL.",
        "members": {
...
_FAMILY_KW = {
    ...
    "audit": ["аудит", "1с", "проводк", "счёт-фактур", "счет-фактур", "фсбу", "вго", "внутригрупп", "себестоимост", "дебиторк", "кредиторк", "инн", "субконто", "план счетов", "первичк", "расхожд"],
...
def audit1c_run_checks(g: dict = None) -> list:
    ...
    def add(fid, cls, check, sev, doc, desc, evidence, chain=None):
        F.append({"id": fid, "класс": cls, "проверка": check, "серьёзность": sev,
                  ...
                  "сумма": (_num(doc.get("СуммаДокумента") or doc.get("Сумма")) if doc else None),
                  "цепочка": chain or []})

    def link(kind, doc=None, present=True, note=""):
        return {"звено": kind, "есть": bool(present), "документ": _ref_short(doc) if doc else None,
    ...
    child = lambda d, t: next((x for x in ref_by.get(d.get("id"), []) if x.get("тип") == t), None)  # noqa: E731
    for d in by_type.get("РеализацияТоваровУслуг", []):
        if d.get("id") not in sf_out_bases:
            pay = child(d, "ПоступлениеНаРасчётныйСчёт")
            add("B1-" + str(d.get("Номер")), "B", "Реализация без счёта-фактуры выданного", "высокая", d,
                ...
                [link("Реализация", d, True, "СуммаДокумента " + str(d.get("СуммаДокумента") or d.get("Сумма") or "")),
                 link("Счёт-фактура выданный (НДС)", None, False, "счёта-фактуры нет"),
                 link("Оплата · взаиморасчёты", pay, bool(pay), "" if pay else "поступления на р/с нет")])
    for d in by_type.get("ПоступлениеТоваровУслуг", []):
        if d.get("id") not in sf_in_bases:
            pay = child(d, "СписаниеСРасчётногоСчёта")
            add("B2-" + str(d.get("Номер")), "B", "Поступление без счёта-фактуры полученного", "высокая", d,
                ...
                [link("Поступление", d, True), link("Счёт-фактура полученный (вычет НДС)", None, False, "счёта-фактуры нет"),
                 link("Оплата поставщику", pay, bool(pay), "" if pay else "списания с р/с нет")])
        ...
        for d in by_type.get(pay_type, []):
            if not d.get("Основание_uuid"):
                add("B3-" + str(d.get("Номер")), "B", "Оплата без документа-основания", "средняя", d,
                    ...
                    [link("Документ-основание (поставка/реализация)", None, False, "ДокументОснование пустое"), link("Платёж", d, True)])
    for d in by_type.get("ПоступлениеТоваровУслуг", []):
        if d.get("id") not in paid_receipts:
            sf = child(d, "СчётФактураПолученный")
            add("B4-" + str(d.get("Номер")), "B", "Поступление не оплачено (кредиторка)", "средняя", d,
                ...
                [link("Поступление", d, True), link("Счёт-фактура полученный", sf, bool(sf)), link("Оплата поставщику", None, False, "списания с р/с нет")])
    for d in by_type.get("РеализацияТоваровУслуг", []):
        if d.get("id") not in paid_sales:
            sf = child(d, "СчётФактураВыданный")
            add("B5-" + str(d.get("Номер")), "B", "Реализация не оплачена (дебиторка)", "средняя", d,
                ...
                [link("Реализация", d, True), link("Счёт-фактура выданный", sf, bool(sf)), link("Оплата покупателя", None, False, "поступления на р/с нет")])
    ...
    for d in by_type.get("РеализацияТоваровУслуг", []):
        ...
        if "90.02" not in codes:
            add("A1-" + str(d.get("Номер")), "A", "Реализация без списания себестоимости", "высокая", d,
                ...
                [link("Выручка Дт 62 Кт 90.01", d, True, "коды Дт: " + ", ".join(sorted(c for c in codes if c))),
                 link("Себестоимость Дт 90.02 Кт 41", None, False, "проводки нет")])
    ...
    for inn, group in ctr_by_inn.items():
        if len(group) > 1:
            ...
            add("C1-" + inn, "C", "Дубль контрагента по ИНН", "высокая", None,
                ...
                [{"звено": "Контрагент «" + str(names[0]) + "»", "есть": True, "документ": None, "статус": "есть", "примечание": "ИНН " + inn}]
                + [{"звено": "Контрагент «" + str(n) + "»", "есть": True, "документ": None, "статус": "ДУБЛЬ", "примечание": "тот же ИНН " + inn} for n in names[1:]])
        ...
        for d in by_type.get(dt, []):
            ...
            if ctr_inn and ctr_inn in org_inns:
                add("D1-" + str(d.get("Номер")), "D", "Внутригрупповой оборот (ВГО)", "высокая", d,
                    ...
                    [link(str(dt) + " (" + str(d.get("Организация") or "орг. А") + ")", d, True),
                     {"звено": "Зеркальный документ у «" + str(org_inns[ctr_inn]) + "»", "есть": True, "документ": None, "статус": "н/д", "примечание": "проверить у второй организации группы"},
                     link("Элиминация при консолидации", None, False, "оборот не помечен как ВГО")])
            ...
            if base and d.get("Дата") and base.get("Дата"):
                if str(d["Дата"])[:4] != str(base["Дата"])[:4]:
                    add("D2-" + str(d.get("Номер")), "D", "Переходящая операция (разные периоды)", "средняя", d,
                        ...
                        [link("Основание (" + str(base["Дата"])[:4] + ")", base, True), link("Оплата (" + str(d["Дата"])[:4] + ")", d, True),
                         {"звено": "Отражение на границе периода", "есть": True, "документ": None, "статус": "ПРОВЕРИТЬ", "примечание": "cut-off 31.12"}])
