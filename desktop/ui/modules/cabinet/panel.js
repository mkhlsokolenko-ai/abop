...
export async function mount(root, ctx) {
  ...
  // профиль пользователя из ABOP (отдел/роли/уровень) + рантайм LLM — честные данные, без курсовых квот
  let u = {}; try { u = await api(C + "/usage"); } catch {}
  const ok = u && u.ok;
  const myRoles = (ok && u.roles && u.roles.length) ? u.roles : ((ctx.roles && ctx.roles.length) ? ctx.roles : ["manager"]);
  ...
  const kpis = [
    ["отдел (ABAC)", esc(dept), "область доступа к данным", "var(--ink)"],
    ["уровень", esc(String(level)), "потолок автономии", "var(--accent-ink)"],
    ["роли", esc(myRoles.join(", ")), "RBAC · realm abop", "var(--ink)"],
    ["LLM рантайм", esc(String(llm)), "self-host ≈ 0 ₽/вызов", "var(--ink)"],
  ];
  ...
  const kpi = (k) => `<div style="padding:17px;border-radius:14px;background:var(--panel);border:1px solid var(--line);backdrop-filter:blur(16px);display:flex;flex-direction:column;gap:6px">
    <span style="${LBL}">${esc(k[0])}</span><span style="font-size:20px;font-weight:800;letter-spacing:-.6px;color:${k[3]};overflow:hidden;text-overflow:ellipsis">${k[1]}</span>
  ...
  // источники из ABOP /sources (реальный статус), fallback — эталонный макет
  let srcList = []; try { const s = await api(C + "/sources"); srcList = (s && s.connectors) || []; } catch {}
  const sources = (srcList.length
    ? srcList.map((s) => ["◵", s.title, s.id, s.status === "ready"])
    : SOURCES
  ).map((s) => { const on = s[3]; return `<div style="display:flex;align-items:center;gap:12px;padding:11px 13px;border-radius:11px;background:var(--hover);border:1px solid var(--line)">
    ...
    <span class="chip ${on ? "on" : ""}">${on ? "готов" : "скоро"}</span></div>`; }).join("");
  ...
  root.innerHTML = `<div style="flex:1;min-width:0;overflow-y:auto;padding:26px 30px">
    <div style="max-width:1020px;margin:0 auto;display:flex;flex-direction:column;gap:22px;animation:ape-in .35s ease-out">
      ...
      <div style="${CARD};flex-direction:row;align-items:center;gap:12px">
        <span style="font-size:15px">📊</span>
        <span style="flex:1;font-size:12.5px;color:var(--ink-2)">Детальный расход токенов и стоимость — в наблюдаемости ABOP (Grafana/метрики). На self-host Qwen стоимость вызова ≈ 0 ₽.</span>
      </div>
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <div style="flex:1 1 380px;min-width:320px;${CARD}">
          ...
          <span style="font-size:11px;color:var(--ink-3)">Роли приходят из Keycloak (твой JWT, realm abop). Энфорс — в ABOP (RBAC/ABAC).</span>
        </div>
